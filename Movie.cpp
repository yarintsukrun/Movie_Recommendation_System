...
Movie::Movie(std::string  name, size_t year):
_name(std::move(name)), _year(year){}
...
std::ostream& operator<<(std::ostream& os,const Movie& sp_movie) {
    os << sp_movie.get_name() << "(" << sp_movie.get_year()
    << ")" << std::endl;
    return os;
}
